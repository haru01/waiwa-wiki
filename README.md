...
試行錯誤の ReAct ループを通じて、**キャリアの目的・制約・使命が本人の中に事後的に立ち上がること**を支援する個人向け LLM-wiki。日々の壁打ち・試行を沈殿させて**確信を育て**、停滞したら**意図的に確信を揺らして探索域を開き直す**。目的が立ち上がったら目的達成型エージェントへ接続する。

`haru01/hypothesis-wiki`（スタートアップの仮説検証 CPF→PMF）を写像元に、キャリア目的形成ドメインへフォークしたもの。

## コアの考え方

- **目的は事前に見つけるものではなく、試行錯誤で事後的に立ち上がる。** 最初の確信度は低くてよい。
